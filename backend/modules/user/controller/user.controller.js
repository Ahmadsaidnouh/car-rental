...
const getCarStatus = (req, res, next) =>
{
    ...
    let all = `SELECT DISTINCT car_id, car_plate, country FROM car ;`
...
const getAvailableCars1 = (req, res, next) =>
{
    let { dropDate, pickDate } = req.body;

    let all;
    all = `select car_id from car`
    let reserved = `select distinct car_id from reservation where ((start_date between '${pickDate}' and '${dropDate}') or (end_date between '${pickDate}' and '${dropDate}') or ((start_date <= '${pickDate}') and (end_date >= '${dropDate}')) or (start_date >= '${pickDate}' and end_date <= '${dropDate}')) and (reserv_status='not-picked' or reserv_status='picked')`
    let outOfService = `select car_id from outOfService where ((start_date between '${pickDate}' and '${dropDate}') or (end_date between '${pickDate}' and '${dropDate}') or ((start_date <= '${pickDate}') and (end_date >= '${dropDate}')) or (start_date >= '${pickDate}' and end_date <= '${dropDate}'))`
    let allCars = []
    let reservedCars = []
...
const addCar = (req, res, next) =>
{
    ...
    query.execute(`INSERT INTO car(company, model, year, category, transmission, car_plate, country, park_city, park_location, rental_charge, car_photo) VALUES ('${company}','${model}','${year}','${category}','${transmission}','${plate}','${country}','${parkCity}','${parkLocation}','${rentalCharge}','${photo}')`, (err, data) =>
    {
        if (err)
            res.json({ message: "error", err })
        else
...
const editCar = (req, res, next) =>
{
    ...
    query.execute(`INSERT INTO outofservice(car_id, start_date, end_date, car_plate) VALUES ('${car_id}','${startDate}','${endDate}','${car_plate}')`, (err, data) =>
    {
        if (err)
            res.json({ message: "error", err })
        else
...
{
    let { email, password } = req.body;
    let userQuery = `select admin_id, password from admin where email='${email}' and password='${password}'`
    query.execute(userQuery, (err, data) =>
    {
